...
pub fn compile_array_contains<'ctx>(cg: &mut CodeGen<'ctx>, ptr: PointerValue<'ctx>, var_name: &str, val_i64: BasicValueEnum<'ctx>, _elem_tag: u64) -> BasicValueEnum<'ctx> {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
...
pub fn compile_array_reverse<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, _elem_tag: u64) {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
    let zero = i64_ty.const_zero();
...
pub fn compile_array_sort<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, elem_tag: u64) {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
    let one = i64_ty.const_int(1, false);
...
pub fn compile_array_insert<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, _elem_tag: u64, idx_expr: &Expr, val_expr: &Expr) -> Result<(), AzError> {
    ...
    let realloc_args = &[ptr.into(), new_sz.into()];
...
pub fn compile_array_map<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, _elem_tag: u64, fn_name: &str) -> Result<BasicValueEnum<'ctx>, AzError> {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
    ...
    let new_ptr = cg.builder.build_array_malloc(i64_ty, len, "map_arr").unwrap();
    let zero = i64_ty.const_zero();
    let one = i64_ty.const_int(1, false);
    let cf = cg.function.unwrap();
...
pub fn compile_array_filter<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, _elem_tag: u64, fn_name: &str) -> Result<BasicValueEnum<'ctx>, AzError> {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
    let fn_val = cg.module.get_function(fn_name).ok_or_else(|| AzError::new(ErrorKind::Semantic, Span::new(0, 0, 0, 0), format!("unknown function '{}'", fn_name)))?;
    let new_ptr = cg.builder.build_array_malloc(i64_ty, len, "flt_arr").unwrap();
    let zero = i64_ty.const_zero();
    let one = i64_ty.const_int(1, false);
    let cf = cg.function.unwrap();
...
pub fn compile_array_reduce<'ctx>(cg: &mut CodeGen<'ctx>, var_name: &str, ptr: PointerValue<'ctx>, _elem_tag: u64, init_expr: &Expr, fn_name: &str) -> Result<BasicValueEnum<'ctx>, AzError> {
    let i64_ty = cg.context.i64_type();
    let len_ptr = cg.array_lengths[var_name];
    let len = cg.builder.build_load(i64_ty, len_ptr, "len").unwrap().into_int_value();
    let fn_val = cg.module.get_function(fn_name).ok_or_else(|| AzError::new(ErrorKind::Semantic, Span::new(0, 0, 0, 0), format!("unknown function '{}'", fn_name)))?;
